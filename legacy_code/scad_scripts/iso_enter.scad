...
module iso_enter_solid(
  base_unit = undef, // base layer value of u(nit) in mm 
  top_unit = undef, // top layer value of u(nit) in mm if different from base layer
  ...
  assert(base_unit != undef, "base_unit is required");
  ...
  if (top_unit != undef) {
    ...
    bottom_path = iso_enter_path(base_unit, fillet_radius_base, skinny);
    top_path = move(xy_offset, iso_enter_path(top_unit, fil_top, skinny));
    skin_path(bottom_path, top_path, height, n_layers);
  } else {
    // get the paths as points
    bottom_path = iso_enter_path(base_unit, fillet_radius_base, skinny);
    top_path = move(xy_offset, iso_enter_path(base_unit, fil_top, skinny));
    skin_path(bottom_path, top_path, height, n_layers);
  }
}
