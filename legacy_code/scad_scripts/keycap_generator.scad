...
KEY_PROFILE = "think"; // [riskeycap, gem, dsa, dcs, dss, kat, kam, xda, think]
...
module bigTest(exportType = 0) {
  ...
  if (exportType == 0) {
    ...
    // for manual multi-material print (with think profile)
    union() {
      color("red") C_Shape();
      translate([0, 0, 0.075]) // Translate a tiny bit up to avoid geometry issues do to rounding
        D_Shape();
    }
  } else {
