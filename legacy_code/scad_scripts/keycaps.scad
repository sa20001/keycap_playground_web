            ...
            translate(
              ...
              rotate([tilt_below_straight, 0, 0]) {
                if (polygon_edges == 4) {
                  ...
                  squarish_poly(base_dimension=xy, h=0.01, r=corner_radius_below, $fn=dish_corner_fn);
                } else {
            ...
            translate(
              ...
              rotate([tilt_above_straight, 0, 0]) {
                if (polygon_edges == 4) {
                  ...
                  squarish_poly(
                    base_dimension=xy, h=0.01, r=corner_radius_above,
                    $fn=dish_corner_fn
                  );
                } else {
                  // We're doing something funky!
              ...
              translate(
                ...
                rotate([tilt_below_straight, 0, 0]) {
                  if (polygon_edges == 4) {
                    ...
                    xy = [length - curve_val_below, width - curve_val_below];
                    squarish_poly(
                      base_dimension=xy, h=0.01,
                      r=corner_radius_below,
                      $fn=dish_corner_fn
                    );
                  } else {
                    // We're doing something funky!
      ...
      if (dish_type == "inv_pyramid") {
        rotate([tilt_above_curved, 0, 0])
          translate([dish_x + top_x, dish_y + top_y, height - dish_depth + dish_z - z_adjust - 0.1])
            rotate([tilt_above_straight, 0, 0])
              squarish_poly(
                base_dimension=[0.01, 0.01],
                top_dimension=[length - top_difference + 0.5, width - top_difference + 0.5],
...
/*
  ...
  Generates a complete keycap mesh (including the internal cavity).
  ...
  Parameters (most have sensible defaults):
    ...
    shape_only             - if true, no cut out, no homing-- just the shape of the keycap
*/
...
module poly_keycap(
  ...
  shape_only = false, // If true, no cut out, no homing-- just the shape of the keycap
) {
  layer_tilt_adjust = dish_tilt / polygon_layers;
      ...
      if (uniform_wall_thickness) {
        ...
        difference() {
          ...
          translate([0, 0, -0.001]) difference() {
              squarish_poly(
                base_dimension=[length - wall_thickness * 2, width - wall_thickness * 2],
                top_dimension=[
  ...
  if (homing_dot_length && homing_dot_width && !shape_only) {
    // Add "homing dots"
    ...
    translate([homing_dot_x, homing_dot_y, height - dish_depth + homing_dot_z - 0.5])
      squarish_poly(
        base_dimension=[homing_dot_length, homing_dot_width],
        h=dish_depth, r=dot_corner_radius,
      );
  }
}
