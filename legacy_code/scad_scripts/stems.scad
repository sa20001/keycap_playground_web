...
/*
Generate a construction cube that sits above the stem top
*/
module topCube(
  ...
  assert(stem_inset != undef, "stem_inset parameter is required");
  assert(stem_height != undef, "stem_height parameter is required");
...
module stem_box_cherry(
  ...
  module stemMain() {
    ...
    difference() {
      translate([0, 0, stem_inset])
        squarish_poly(base_dimension=[length, width], h=stem_height, r=stem_corner_radius);
      translate([0, 0, stem_inset])
        cherry_cross(tolerance=inside_tolerance, flare_base=true);
    }

    // BOX CHERRY TOPPER
    translate([0, 0, stem_inset + stem_height]) {
      squarish_poly(
        base_dimension=[length, width],
        top_dimension=[length, width],
        h=stem_topper_height,
        r=stem_corner_radius
      );
    }
    ...
    if (stem_flat_support && stem_inset > 0) {
      ...
      translate([0, 0, -support_distance]) difference() {
          ...
          translate([0, 0, stem_inset / 2 + support_distance / 2])
            squarish_poly(base_dimension=[length, width], h=stem_inset - support_distance, r=stem_corner_radius);
          translate([0, 0, stem_inset / 2])
            squarish_poly(base_dimension=[length - extrusion_width * 2, width - extrusion_width * 2], h=stem_inset + 1, r=stem_corner_radius);
          translate([0, 0, stem_inset / 2])
  ...
  union() {
    ...
    fillet_radius = 1.5;
    path_radius = 3;
    Z_pos = stem_height + stem_inset + cubeZ_Offset - fillet_radius;

    // pointA_x = length + fillet_radius + path_radius;
    pointA_x = length + fillet_radius * 2;
    pointA_y = 0;
    pointB_x = pointA_x;
    pointB_y = width + fillet_radius * 2;
    pointC_x = 0;
    pointC_y = pointB_y;

    echo(Z_pos);
    echo(path_radius);
    echo(stem_corner_radius);
    translate([0, 0, Z_pos])
      stem_fillet(
        ...
        path_fillet_radius=path_radius,
        flip=[0, 0, 0],
        internal=true
      );
  }
    ...
    if (stem_flat_support && stem_inset > 0) {
      translate([0, 0, -support_distance]) difference() {
          ...
          translate([0, 0, stem_inset / 2 + support_distance / 2])
            squarish_poly(base_dimension=[length, width], h=stem_inset - support_distance, r=stem_corner_radius);
          translate([0, 0, stem_inset / 2])
            squarish_poly(base_dimension=[length - extrusion_width * 3, width - extrusion_width * 3], h=stem_inset + 1, r=stem_corner_radius);
          translate([0, 0, stem_inset / 2])
  ...
  difference() {
    ...
    if (uniform_wall_thickness) {
      ...
      squarish_poly(
        ...
        xy_offset=[top_x, top_y],
        h=key_height, r=key_corner_radius / 2
      );
    }
      ...
      for (loc = [0:1:len(locations) - 1]) {
        translate([locations[loc][0], locations[loc][1], key_height / 2 + locations[loc][2] - 0.01])
          squarish_poly(
            ...
            h=key_height, r=stem_corner_radius
          );
      }
      ...
      for (loc = [0:1:len(locations) - 1]) {
        ...
        translate([locations[loc][0], locations[loc][1], key_height / 2 + locations[loc][2] - 0.01]) union() {
            squarish_poly(
              ...
              h=key_height, r=stem_corner_radius / 4
            );
            translate([0, 0, -support_distance / 2]) squarish_poly(
                base_dimension=[alps_length, alps_width],
                top_dimension=[
                  alps_length * 1.5 + support_distance * 2,
                  alps_width * 2.5 + support_distance * 2,
                ],
                h=key_height - ALPS_STEM_DEPTH - stem_depth + top_thickness, r=key_corner_radius / 2
              );
            // This cuts off some of the excess:
            translate([0, 0, key_height / 2 + support_distance * 2]) squarish_poly(
                base_dimension=[
                  key_length,
                  key_width,
                ],
